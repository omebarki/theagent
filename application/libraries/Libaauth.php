...
class Libaauth {
	...
	/**
	 * generate ACLs for user_id
	 * 
	 */
	public function generateACL($user_id){
		$acl = $perm_ids = array();
		//get group_id
		...
		foreach($roles as $role_id){
			$perms = array_map(
				function(stdClass $o){return $o->perm_id;},
				$this->get_role_perms($role_id)
			);
			$perm_ids = array_merge($perm_ids, $perms);
		}
		// add individual perms for group & melt
		// add individual perms for user & melt
		$perms = array_map(
			function(stdClass $o){return $o->perm_id;},
			$this->get_user_perms($user_id)
		);
		$perm_ids = array_unique(array_merge($perm_ids, $perms));
		// get Acl codes
		foreach($perm_ids as $perm_id){
			$acl[] = $this->get_perm_name($perm_id);
		}
		//set in session
	...
	public function get_group_roles($group_id){
		...
		$this->CI->db->select('*');
		$this->CI->db->from($this->config_vars['role_to_group']);
		$this->CI->db->join($this->config_vars['roles'], "id = role_id");
	...
	public function get_user_roles($user_id){
		...
		$this->CI->db->select('*');
		$this->CI->db->from($this->config_vars['role_to_user']);
		$this->CI->db->join($this->config_vars['roles'], "id = role_id");
		$this->CI->db->where('user_id', $user_id);
	...
	public function get_role_perms($role_id){
		...
		$this->CI->db->select('*');
		$this->CI->db->from($this->config_vars['perm_to_role']);
		$this->CI->db->join($this->config_vars['perms'], "id = perm_id");
	...
	public function get_user_perms($user_id){
		...
		$this->CI->db->select('*');
		$this->CI->db->from($this->config_vars['perm_to_user']);
		$this->CI->db->join($this->config_vars['perms'], "id = perm_id");
		$this->CI->db->where('user_id', $user_id);
	 ...
	 * @return int Permission id or NULL if perm does not exist
	 */
	public function get_perm_name($perm_id) {
