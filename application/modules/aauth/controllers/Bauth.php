 ...
 * @version 1.0
 */
class Bauth extends MX_Controller {

    public function __construct() {
        parent::__construct();
        // Your own constructor code
        $this->load->library("libaauth",NULL,'aauth');
        $this->load->library('form_validation');
        $this->lang->load("aauth",$this->session->userdata('site_lang'));
    }
/**
*----------------------------ACTIONS---------------------------------------
*/
    ...
    public function index() {
        if ($this->is_loggedin()){
            ...
            $this->load->view('login_back_tpl', array(
                'login_error' => implode('<br/>', $this->aauth->get_errors_array()),
            ));
        }
    }
        ...
        if($this->input->is_ajax_request()){
            ...
            if ($this->form_validation->run() != FALSE){
                if ($this->aauth->login(
                    ...
                    if($this->checkAccess('role','ta_worker')){
                        $redirect = '/backoffice/dashboard/';
                    }
                    else{
                        $redirect = '/aauth/Bauth/logout';
                    }
                }
            }
            $this->output
            ...
            ->set_output(json_encode(array(
                'msg' => array(
                    ...
                    'login_error' => $this->aauth->get_errors_array(),
                ),
                'redirect' => $redirect
            )));
        }
    ...
    public function forgot() {
        if($this->input->is_ajax_request()){
            ...
            if ($this->form_validation->run() != FALSE){
                ...
                $msg = $this->lang->line("aauth_email_verification_code_sent");
            }
            $this->output
            ->set_content_type('application/json')
            ->set_output(json_encode(array(
